...
"""
    ...
    def __update_provisioner(self):
        ...
        with open(provisioner_path, encoding="utf-8") as stream:
            provisioner: ProvisionerConfig = yaml.safe_load(stream)    
        ...
        for variant in self.info.board_variants:
            ...
            if variants_combos[variant] not in provisioner["devices"][device_idx]["variants"][variant_idx]["supported_combos"]:
        ...
        with open(provisioner_path, "w", encoding="utf-8") as f:
            yaml.dump(provisioner, f, default_flow_style=False)
    ...
    def __do_one_sys(self, info: SystemInfo,
        ...
        for file in files:
            ...
            for image_name, image_manifests in self.index.items():
                ...
                if util.cmp_version(newest_version, current_version) > 0:
                    needs_update = True
            ...
            if not file_exists:
                ...
                info.update_info.stat = "update"
                info.update_info.file_stat[file] = (
                    "update", f"0.0.0 -> {plug.handle_version(info)}")
        if config["force"]:
            needs_update = True
        ...
        if not needs_update:
            return
        yield SystemWorker(info, plug, repo)
    ...
    def gen_branch(self, repo: RuyiGitRepo) -> Generator[SystemWorker, None, None]:
        ...
        for info in self.oldver:
            ...
            try:
                ...
                info.update_info.stat = "eutil.system_id(info, 'update_branch')rror"
                logger.error(
                    "Error occurs when handling system %s", repr(info))
                logger.error(traceback.format_exc())

    def update_info(self) -> list[tuple[str, str]]:
        """
