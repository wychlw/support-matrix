...
Get the source code following the official tutorial:
```bash
...
Write the source code to the SD card:
```bash 
...
Note: It has not been merged into the mainline yet, specific repository is needed to fetch Zephyr:
```bash
...
The secondary core where Zephyr is located uses UART1 (GP1: TX, GP2: RX, GP3: GND)

## Expected Results

The system should start up normally, and information can be viewed through the onboard serial port.
